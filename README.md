...
## 测试
```bash
.venv/bin/python -m pytest tests/ -q   # 172 个用例全绿（仅测试允许 mock）
```
...
- ⏭ 下一步可选：拍照/截图记账（多模态，需视觉模型）· 本地唤醒词模型（keyword spotter）· 真 Web Push · RAG · Telegram 入口 · 技能库向量检索 / 失败触发主动提议 / 工具运行反馈环
