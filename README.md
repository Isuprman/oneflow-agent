...
| 分句流式播报 | 回复按句切分，首句合成完即开播（出声延迟从“全文”降到“首句”）；单句合成超 8 秒自动回退浏览器语音；文字与声音对齐（播完才淡出） |
...
## 测试
```bash
.venv/bin/python -m pytest tests/ -q   # 146 个用例全绿（仅测试允许 mock）
```
...
- ✅ 语音交互升级：会话模式（唤醒即长对话/退下词退出/闲置告退）· 递进式俏皮话 · 分句流水线播报（首句即播/8s 超时回退/声画对齐）
